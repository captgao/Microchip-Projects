...
# 1 "mcc_generated_files/mtouch/mtouch.h" 1
# 91 "mcc_generated_files/mtouch/mtouch.h"
...
enum mtouch_button_state
{
    ...
    const mtouch_button_t mtouch_button_init[(u_int)1] =
    {
        ...
        { Button0,
            ...
            0,0,0,0,0,
            (mtouch_button_deviation_t)100,
            (mtouch_button_scaling_t)1
        }
    };

    static mtouch_button_t mtouch_button[(u_int)1];
...
void MTOUCH_Button_InitializeAll(void)
{
    ...
    for (button = 0; button < (u_int)1; button++)
    {
...
static void Button_State_Initializing(mtouch_button_t* button)
{
    ...
    if ((button->counter) <= (mtouch_button_statecounter_t)((mtouch_button_statecounter_t)16)/2)
    {
    ...
    (button->counter)++;
    if ((button->counter) >= (mtouch_button_statecounter_t)((mtouch_button_statecounter_t)16))
    {
        button->reading = (mtouch_button_reading_t)((button->baseline)>>((uint8_t)4));
...
static void Button_State_NotPressed(mtouch_button_t* button)
{
    ...
    if ((button->deviation) < (mtouch_button_deviation_t)(mtouch_button_deviation_t)-10)
    {
        (button->counter)++;
        if ((button->counter) > (mtouch_button_statecounter_t)((mtouch_button_statecounter_t)32))
        {
            button->counter = (mtouch_button_statecounter_t)0;
            MTOUCH_Button_Initialize(button->name);
        }
    ...
    if ((button->baseline_count) > ((mtouch_button_baselinecounter_t)32))
    {
        button->baseline_count = (mtouch_button_baselinecounter_t)0;
        Button_Baseline_Update(button);
    }
...
static void Button_State_Pressed(mtouch_button_t* button)
{
    ...
    if ((button->counter) >= ((mtouch_button_statecounter_t)1000))
    {
        MTOUCH_Button_Initialize(button->name);
...
{
    if(name < (u_int)1)
        return (mtouch_button[name].state == MTOUCH_BUTTON_STATE_pressed) ? 1 : 0;
    else
        return 0;
}
...
{
    if(name < (u_int)1)
        return (mtouch_button[name].state == MTOUCH_BUTTON_STATE_initializing) ? 0 : 1;
    else
        return 0;
}
...
static void Button_Deviation_Update(mtouch_button_t* button)
{
    ...
    deviation = (int32_t)((int32_t)(button->reading) - (int32_t)((button->baseline)>>((uint8_t)4)));
...
static void Button_Reading_Update(mtouch_button_t* button)
{
    ...
    {
        GIE = (uint8_t)0;
        button->reading -= button->reading>>(uint8_t)2;
        button->reading += MTOUCH_Sensor_RawSample_Get(button->sensor)>>(uint8_t)2;
        GIE = (uint8_t)1;
    }
    else
    {
...
static void Button_Baseline_Update(mtouch_button_t* button)
{
    ...
    {
        GIE = (uint8_t)0;
        button->baseline -= (button->baseline) >> ((uint8_t)4);
...
{
    return (mtouch_button_reading_t)((mtouch_button[name].baseline) >> ((uint8_t)4));
}
# 448 "mcc_generated_files/mtouch/mtouch_button.c"
static void Button_DefaultCallback(enum mtouch_button_names button) { }
